...
- 连续失败达到 `COOLDOWN_THRESHOLD`（默认 5）次 → 设置 `cooldown_until = now + COOLDOWN_MS`（默认 60s），并把计数清零。
- 任意一次成功 → 立即重置连续失败计数（不等冷却）。
- 冷却期内该 key 被跳过；冷却结束后自动回到候选池。
- breaker 计数本身有 10 分钟 TTL——10 分钟内无新失败则视为"该 key 已恢复"，避免历史失败持续压制。

### 6.3 429 重试

- 首次 429：**不**直接视为失败，先切到另一个候选 key 重试一次。
- 重试仍 429：两个 key 各记一次失败，原样透传上游响应。
- 重试成功：走 2xx 路径，第一个 key 也记失败（首次 429 本就是失败信号）。
