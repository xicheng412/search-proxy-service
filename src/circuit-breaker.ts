...
export async function recordUpstreamSuccess(
  ...
  const { postUseCooldownSec } = await config.get(kv);
  await setUpstreamCooldown(kv, def, id, now + postUseCooldownSec * 1000).catch(() => {});
...
export async function recordUpstreamFailure(
  ...
  const { postUseCooldownSec, breakerBaseSec } = await config.get(kv);
...
export async function recordUpstreamInvalid(
  ...
  const { postUseCooldownSec, invalidCooldownSec } = await config.get(kv);
