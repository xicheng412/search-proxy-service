...
// 冷却共用一个 cooldown_until 字段，写入时取较大值：
//   1. Post-use 冷却：每次使用后（无论成败）固定时长（默认 10s，可调）
//   2. 熔断冷却：仅 server-error 族失败后（5xx / 网络 / 2xx-不可用），指数退避 = base × 2^连续失败次数（base 默认 10min，可调）
//   3. 疑似失效冷却：401/403（auth-error）与 429/432（rate-limit）× key 级不可用，固定 invalidCooldownSec（默认 12h，可调），不碰连续失败计数
...
export function computeBreakerOutcome(
  ...
  switch (result) {
    ...
    case "rate-limit":
    case "auth-error":
      // key 级不可用（429/432 限流额度耗尽 / 401/403 鉴权错误）：复用疑似失效的固定长冷却
      // （invalidCooldownSec，默认 12h），以 post-use 为地板；不碰熔断连续计数。额度耗尽的
      // key 由此进入长冷却，不再被反复选中打真实请求。两族同机制、独立 cause 便于 UI 标注。
      return {
        consecutive: null,
        cooldownUntil: now + Math.max(postUseSec, invalidSec) * 1000,
        cause: result === "auth-error" ? "invalid" : "rate-limit",
      };
    case "server-error": {
