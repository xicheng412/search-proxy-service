...
exaAdmin.post("/batch-toggle", async (c) => {
  const body = await c.req.parseBody();
  const ids = normalizeBatchIds(body["ids[]"]);
  if (ids.length === 0) return c.html(errorFragment("未选择任何 key"));
  const env = c.env;
  const missing = await missingUpstreamKeyIds(env, EXA.upstream, ids);
  if (missing.length) {
    return c.redirect(
      `/admin/exa?flash=${encodeURIComponent(`已选 ${ids.length} 个，其中 ${missing.length} 个不存在，未切换任何 key`)}`,
      303
    );
  }
  const changed = await toggleUpstreamKeysBatch(env, EXA.upstream, ids);
  for (const id of ids) await notifyKeyPoolActivate(c.env, EXA.name, id).catch(() => {});
  await notifyKeyPoolSync(c.env, EXA.name).catch(() => {});
  return c.redirect(
    `/admin/exa?flash=${encodeURIComponent(`已切换 ${changed} 个 key`)}`,
    303
  );
});

// 批量删除：存在性预查，缺失即整批拒绝；单条 DELETE；全量合并 status。
exaAdmin.post("/batch-delete", async (c) => {
  const body = await c.req.parseBody();
  const ids = normalizeBatchIds(body["ids[]"]);
  if (ids.length === 0) return c.html(errorFragment("未选择任何 key"));
  const env = c.env;
  ...
  if (missing.length) {
    return c.redirect(
      `/admin/exa?flash=${encodeURIComponent(`已选 ${ids.length} 个，其中 ${missing.length} 个不存在，未删除任何 key`)}`,
      303
    );
  }
  const changed = await deleteUpstreamKeysBatch(env, EXA.upstream, ids);
  await notifyKeyPoolSync(c.env, EXA.name).catch(() => {});
  return c.redirect(
    `/admin/exa?flash=${encodeURIComponent(`已删除 ${changed} 个 key`)}`,
    303
  );
});
