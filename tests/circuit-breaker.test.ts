// 熔断/冷却数学单测：经 KeyPool 内存池断言 recordUpstreamOutcome 的指数退避、
// 10min 空窗复位与三层冷却语义（server-error / success / rate-limit / auth-error 四族）。
// 默认配置（postUse 10s / base 600s / invalid 43200s）由 fakeKV.get→null 稳定给出。
...
import { recordUpstreamOutcome } from "../src/circuit-breaker";
...
describe("recordUpstreamOutcome server-error 指数退避", () => {
  ...
  it("窗口内二次失败：consecutive=2，冷却 = now + max(post-use, base×2^2)", async () => {
    const { env, pool } = fresh();
    await recordUpstreamOutcome(env, pool, "k1", "server-error", T0);
    await recordUpstreamOutcome(env, pool, "k1", "server-error", T0 + POST_USE_MS);
  ...
  it("超窗失败（距上次 > BREAKER_TTL_MS 10min）：计数复位为 1", async () => {
    const { env, pool } = fresh();
    await recordUpstreamOutcome(env, pool, "k1", "server-error", T0);
    await recordUpstreamOutcome(env, pool, "k1", "server-error", T0 + POST_USE_MS); // updated_at 推进到 T0+10_000
    // 距 updated_at 已超 10min 空窗（> T0+10_000+600_000）→ 视为恢复，从 1 重计
    const beyond = T0 + POST_USE_MS + BASE_MS + 90_000;
    await recordUpstreamOutcome(env, pool, "k1", "server-error", beyond);
