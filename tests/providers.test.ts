...
//   - reader 协议只能落在 Extract 能力上（reader 只包装 Extract，对称于 searxng 只包装 Search）；
//   - exa 不声明 Extract → /extract、/reader 上它是 404 的来源；
//   - 每个声明的 Surface 非退化（有 path、开放至少一个协议）。
// 刻意不断言 "searxng/reader 只在 tavily"：那是当前实现覆盖，不是不变量。
...
describe("能力×协议矩阵", () => {
  ...
  it("reader 永不落在 Extract 以外的能力上", () => {
    for (const provider of Object.values(PROVIDERS)) {
      Object.entries(provider.capabilities).forEach(([cap, surface]) => {
        if (surface.protocols.includes("reader")) expect(cap).toBe("extract");
      });
    }
  });
  ...
  it("tavily 的 Extract 同时开放 native 与 reader", () => {
    expect(TAVILY.capabilities.extract!.protocols).toContain("native");
    expect(TAVILY.capabilities.extract!.protocols).toContain("reader");
  });
