...
describe("KeyPool", () => {
  it("1. applyBreakerOutcome 原地生效：getKeys() 同一对象、cooldown 即刻可见、breaker 记录", () => {
    ...
    pool.applyBreakerOutcome("k1", 5000, 2, 1000);
  ...
  it("2. consecutive===null 不生成 getBreakerState 条目", () => {
    const pool = createKeyPool({ DB: {} } as unknown as Env, def, [key("k1")]);
    pool.applyBreakerOutcome("k1", 5000, null, 1000);
  ...
  it("3. flushNow 批量 checkpoint：一次 DB.batch 写 cooldown_until，dirty 清零后不再写", async () => {
    ...
    pool.applyBreakerOutcome("k1", 5000, 0, 1000);
    pool.applyBreakerOutcome("k2", 6000, 1, 1000);
    await pool.flushNow();
    const l = log();
    ...
    expect(l[2].binds).toEqual([6000, def.provider, "k2"]);
  ...
  it("4. reload 合并：保留内存 cooldown、采纳 admin 的 name/status、清理已删 id 及其计数", async () => {
    const { db } = makeScriptedD1([
      {
        results: [
          { id: "k1", key: "tvly-k1", name: "renamed", status: "disabled", cooldown_until: null, created_at: 1 },
          { id: "k3", key: "tvly-k3", name: "", status: "enabled", cooldown_until: 999, created_at: 2 },
        ],
      },
    ]);
    const pool = createKeyPool({ DB: db } as unknown as Env, def, [
      key("k1", { cooldown_until: 5000 }),
      key("k2"),
    ]);
    pool.applyBreakerOutcome("k2", 7000, 1, 1000); // 将随 k2 删除一并清理
    await pool.reload();
