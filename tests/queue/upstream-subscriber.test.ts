...
// 一次；rate-limit 只冷却不记账；client-error 不处理（不入事件）。回归保护：DO 重建叠加
// 订阅者导致的 usage/冷却翻倍（见 upstream-subscriber.ts 头注释）。
...
describe("makeUpstreamSubscriber", () => {
  ...
  it("rate-limit：仅 post-use 冷却，不记 usage", async () => {
    const env = envOf();
    const store = getUsageStore(env);
    const pool = poolOf(env);
    const sub = makeUpstreamSubscriber(env, () => pool);
    ...
    const at = Date.now();
    await sub({ type: "upstream-attempt-settled", keyId: "k1", provider: "tavily", cls: "rate-limit", at });

    expect(pool.getBreakerState("k1")).toBeNull(); // 不碰连续计数
    expect(pool.getKeys()[0].cooldown_until).toBe(at + 10_000);
    await expect(store.readUpstreamWeightSignal(["k1"])).resolves.toEqual({ k1: 0 });
  });
